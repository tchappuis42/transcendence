...
const PlayerCard: React.FC<cardProps> = ({ idOne, idTwo }) => {
	...
	useEffect(() => {
		const getInfoPlayerOne = async () => {
			try {
				const response = await axios.get(`http://localhost:4000/user/${idOne}`);
		...
		const getInfoPlayerTwo = async () => {
			try {
				const response = await axios.get(`http://localhost:4000/user/${idTwo}`);
	...
	return (
		<div className="bg-white w-full h-20 min-[845px]:w-[750px] rounded-t-xl flex sm:h-32">
			...
			<div className="w-1/2 flex flex-row-reverse">
				<div className="w-[29%]">
					<img src={playerTwo?.avatar} alt="" className="rounded-tr-xl h-full w-auto" />
				</div>
				<div>
